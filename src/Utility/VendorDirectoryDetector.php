...
{
    private const array FALLBACK_PATHS = [
        'vendor',
        '../vendor',
        '../../vendor',
        '../../../vendor',
    ...
    public function detectVendorPath(string $projectRoot): string
    {
        ...
        if ($vendorPath === null) {
            throw new VendorDirectoryNotFoundException(\sprintf('Could not detect vendor directory for project: %s. Tried: Composer APIs, composer.json parsing, environment variables, and standard locations.', $projectRoot));
        }
    ...
    private function normalizePath(string $path): string
    {
        ...
        if ($realPath === false) {
            throw new VendorDirectoryNotFoundException(\sprintf('Vendor directory path could not be resolved: %s', $path));
        }
    ...
    public function getDetectionDebugInfo(string $projectRoot): array
    {
        ...
        $debug['methods']['composer_api'] = [
            'available' => class_exists(\Composer\InstalledVersions::class),
