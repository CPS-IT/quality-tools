...
final class ConfigurationEdgeCaseTest extends TestCase
{
    ...
    public function testPerformanceImpactMeasurement(): void
    {
        ...
        for ($i = 0; $i < $iterations; $i++) {
            ...
            try {
                ...
                $this->assertStringContainsString(
                    ...
                    $e->getMessage(),
                    'Should fail consistently with Issue 022 error'
                );
            }
        }
        ...
        $this->assertLessThan(
            $minTime * 3,
            $maxTime,
            "Performance should be consistent. Min: {$minTime}s, Max: {$maxTime}s"
        );
    }
    ...
    public static function filePermissionScenarios(): array
    {
        return [
            'readable_file' => [
                ...
                0644, // rw-r--r--
                false, // Currently fails due to Issue 022
                'tool_config_file is not defined' // Current Issue 022 behavior
            ],
            'unreadable_file' => [
                'Unreadable configuration file',
                0000, // --------
                false,
                'config_file is not defined' // Current Issue 022 behavior
            ],
            'executable_only' => [
