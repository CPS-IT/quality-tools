...
final class CompleteWorkflowTest extends TestCase
{
    ...
    private function createComplexExtensionStructure(): void
    {
        ...
        foreach ($directories as $dir) {
            mkdir($classesDir . '/' . $dir, 0o777, true);
        }
    ...
    {
        file_put_contents(
            $baseDir . '/Controller/NewsController.php',
            <<<'PHP_WRAP'
                <?php
    ...
    {
        file_put_contents(
            $baseDir . '/Domain/Repository/NewsRepository.php',
            <<<'PHP'
                <?php
    ...
    {
        file_put_contents(
            $baseDir . '/Service/NewsService.php',
            <<<'PHP'
                <?php
    ...
    {
        file_put_contents(
            $baseDir . '/Utility/ArrayUtility.php',
            <<<'PHP'
                <?php
    ...
    {
        file_put_contents(
            $baseDir . '/ViewHelpers/FormatDateViewHelper.php',
            <<<'PHP'
                <?php
    ...
    {
        file_put_contents(
            $baseDir . '/EventListener/NewsEventListener.php',
            <<<'PHP'
                <?php
    ...
    private function createConfigurationFiles(string $extensionDir): void
    {
        ...
        mkdir($extensionDir . '/Configuration/TypoScript', 0o777, true);
        file_put_contents(
            $extensionDir . '/Configuration/TypoScript/setup.typoscript',
            <<<'TS'
    ...
    private function analyzeCodeQuality(): array
    {
        ...
        foreach ($phpFiles as $file) {
            ...
            // Count style issues
            if (str_contains($content, 'if(') || str_contains($content, '){')) {
                ++$issues;
            }
        }
    ...
    private function countPhpStanErrors(string $output): int
    {
        ...
        return str_contains($output, '[OK]') ? 0 : 1;
    }
