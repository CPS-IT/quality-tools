...
final class ErrorRecoveryTest extends TestCase
{
    ...
    private function createProjectStructure(): void
    {
        ...
        mkdir($classesDir . '/Service', 0o777, true);
    ...
    {
        file_put_contents(
            $baseDir . '/Controller/SyntaxErrorController.php',
            <<<'PHP'
                <?php
    ...
    {
        file_put_contents(
            $baseDir . '/Service/LogicErrorService.php',
            <<<'PHP'
                <?php
    ...
    {
        file_put_contents(
            $baseDir . '/Service/ValidService.php',
            <<<'PHP'
                <?php
                namespace ErrorRecovery\Test\Service;
    ...
    {
        file_put_contents(
            $baseDir . '/Controller/PartialController.php',
            <<<'PHP'
                <?php
                namespace ErrorRecovery\Test\Controller;
    ...
    public function testErrorMessageQuality(): void
    {
        ...
        if (!empty($result['output']) && (str_contains((string) $result['output'], 'syntax error') || str_contains((string) $result['output'], 'Parse error'))) {
            // PHPStan may report syntax errors but not necessarily mention specific controller names
            $this->assertTrue(
                str_contains((string) $result['output'], 'SyntaxErrorController')
                || str_contains((string) $result['output'], 'syntax error')
    ...
    private function createAdditionalValidFiles(): void
    {
        ...
        file_put_contents(
            $baseDir . '/Service/AdditionalService.php',
            <<<'PHP'
                <?php
                namespace ErrorRecovery\Test\Service;
